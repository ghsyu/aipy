...
__global__ void sum_max(unsigned int n, float *g_nscore_i, float *g_max_i, int* g_max_idx_i, \
    ...
    /*Load shared memory from global memory, and set out-of-range values to 0
    Since out-of-range values will have a magnitude of 0, they should not interfere
    with the reduction*/
    if (i < n){
    ...
    /* if Re(i)^2 + Im(i)^2 > Re(j)^2 + Im(j)**2, copy j to i (where i and j are elements of s_max)
       ...
       The loop will run until max(Re(i)^2 + Im(i)^2) is the 0th element of s_max, and
       the 0th and 1st elements /of s_max_idx contain the x and y coordinates of the max in the dirty image.
       Only works on power of 2 sized arrays, which means the blocksize must be a power of 2 */
    ...
    //Load the results from shared memory back into global memory
    if (tid == 0){
...
__global__ void clean2dc(unsigned int dim1, unsigned int dim2, unsigned int argmax1, unsigned int argmax2, float stepr, \
    ...
    int *s_max_idx  = (int *) &s_max[2*(blockDim.x*blockDim.y)];
    float valr, vali;
    ...
    if ((n1 < dim1) && (n2 < dim2) && (area[wrap_n1*dim2 + wrap_n2] == 1)){
    ...
    __syncthreads();
    //Do one reduction before handing over to sum_max
    ...
    if (tid == 0){
        g_nscore[bid]        = s_nscore[0];
        g_max[2*bid]         = s_max[0];
        g_max[2*bid+1]       = s_max[1];
        g_max_idx[2*bid]     = s_max_idx[0];
        g_max_idx[2*bid+1]   = s_max_idx[1];
    }
    return;
}
//Helper functions
...
int gpu_set_up(float **dev_ker,    float **dev_res, int **dev_area, \
    ...
    cudaMalloc((void**) g_nscore_i,   sizeof(float)*dim1*dim2);
    cudaMalloc((void**) g_max_i,      2*sizeof(float)*dim1*dim2);
    cudaMalloc((void**) g_max_idx_i,  2*sizeof(int)*dim1*dim2);
...
int copy_res(float *res, float *dev_res, int res_len){
    cudaMemcpy(res, dev_res, res_len, cudaMemcpyDeviceToHost);
    return 0;
}
//   ____ _                  ____     _      
...
float *clean_2d_c_GPU(float *res, float *ker, int * area, \
    ...
    int len = gridx*gridy; //The total number of elements left to reduce
    while(len > 1){
    ...
    cudaMemcpy(nscore_p, g_nscore_o, sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(max_p, g_max_o, 2*sizeof(float), cudaMemcpyDeviceToHost);
