...
class Test(threading.Thread):
    def __init__(self, iterations, device = [0,1,2,3]):
    ...
    def run(self):
        for i in xrange(self.iterations):
            self.cim, self.info = d2.clean(self.dim, self.dbm, tol=1e-5, stop_if_div=True, maxiter=1000, devices=self.device)

class FastTest(Test):
    def run(self):
        for i in xrange(self.iterations):
            self.cim, self.info = d2.clean(self.dim, self.dbm, tol=1e-5, stop_if_dev=True, maxiter = 10, device = self.device)
